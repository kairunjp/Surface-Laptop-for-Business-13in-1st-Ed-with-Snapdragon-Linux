...
DEFAULT_DSP_FIRMWARE_SOURCE="$ROOT_DIR/archlinux/firmware-tree"
...
build_archlinux_dtbs() {
	...
	for current in \
		...
		if fdtget "$current" /audio-codec compatible >/dev/null 2>&1; then
			die "Arch DTB unexpectedly enables the experimental WCD9385 headset codec: $current"
		fi
...
build_surface_kernel_package() {
	...
	if fdtget "$installed_dtb" /audio-codec compatible >/dev/null 2>&1; then
		die "installed target DTB unexpectedly enables the experimental WCD9385 headset codec: $installed_dtb"
	fi
...
main() {
	...
	if [[ -z "$DSP_FIRMWARE_SOURCE" ]]; then
		[[ -d "$DEFAULT_DSP_FIRMWARE_SOURCE" ]] ||
			die "bundled Surface DSP firmware tree is missing: $DEFAULT_DSP_FIRMWARE_SOURCE"
		DSP_FIRMWARE_SOURCE="$DEFAULT_DSP_FIRMWARE_SOURCE"
	fi
